...
void InputManager::PopText()
{
		if (_text.length() > 0)
		{
			_text.pop_back();
			popchar(text, (_text.length()), sizeof(text));
		}
		else
		{
		}
...
void InputManager::popchar(char * c, int x, int size)
{
	...
	while(SDL_PollEvent(&events))
	{
		...
		switch(events.type)
		{
			case SDL_TEXTINPUT:
			{
				if (_text.length() < 20)
				{
					strcat_s(text, sizeof(std::string), events.text.text);
					_text = text;
				}
				break;
			}
